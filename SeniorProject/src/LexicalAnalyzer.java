...
public class LexicalAnalyzer {
	...
	// throws new InvalidArgumentException if fileName is null
	...
	// postcondition: source code line has been processed with all tokens placed in the token list
	...
	// throws LexicalException if lexeme is not a valid representation of a token
	private TokenType getTokenType(String lexeme, int rowNumber, int columnNumber) throws LexicalException {
		...
		else if (Character.isLetter(lexeme.charAt(0)))
		{
			if (lexeme.length() == 1)
			...
			else if (lexeme.equals("Console.Write"))
				...
				tokType = TokenType.STATIC_TOK;
			else if (lexeme.equals("void"))
				tokType = TokenType.VOID_TOK;
			else if (lexeme.equals("class"))
				tokType = TokenType.CLASS_TOK;
			else if (lexeme.equals("Main"))
				tokType = TokenType.MAIN_TOK;
			else if (lexeme.equals("string[]"))
				tokType = TokenType.STRING_ARR_TOK;
			else if (lexeme.equals("args"))
				tokType = TokenType.ARGS_TOK;
			...
			tokType = TokenType.LEFT_BRACKET_TOK;
		else if (lexeme.equals("}"))
			tokType = TokenType.RIGHT_BRACKET_TOK;
		else if (lexeme.equals(">="))
			...
			tokType = TokenType.SEMICOLON_TOK;
		else if (lexeme.equals("%"))
			tokType = TokenType.MODULO_TOK;
		else
			throw new LexicalException ("invalid lexeme "+ " at row " +
					rowNumber  + " and column " + columnNumber);
		return tokType;
	...
	// throws new IllegalArgumertException if lexeme is null
	...
	// index >= 0
	// return next lexeme
	// throws new IllegalArgumentExceptio if line is null or line < 0
	...
	public Token getLookNextToken() throws LexicalException {
		if (tokenList.isEmpty())
			throw new LexicalException ("no more tokens");
